  ...
  const handleAnswerSelect = (answer: string) => {
    ...
    setTimeout(() => {
      if (currentQuestion < questions.length - 1) {
        ...
        router.push(`/quiz/3/results?score=${score}&total=${questions.length}&name=${encodeURIComponent(name)}`)
      }
    }, 1500)
  }
        ...
        <div className="grid grid-cols-1 gap-4">
          {question.options.map((answer, index) => (
            <button
              ...
              disabled={isAnswered}
              className={`${getAnswerButtonClass(answer)} p-4 rounded-lg shadow transition-all duration-200 text-lg font-medium text-gray-900 disabled:cursor-not-allowed`}
            >
              {answer}
            </button>
          ))}
