...
export default function AgePage() {
  ...
  const handleAgeSelect = (age: string) => {
    ...
    router.push(`/quiz/3/${age}?name=${encodeURIComponent(name)}&version=easy`)
  }
        ...
        <div className="grid grid-cols-2 gap-4">
          {Object.entries(ageGroups).map(([age, text]) => (
            <button
              ...
              className="bg-purple-100 hover:bg-purple-200 text-purple-700 font-semibold py-4 px-6 rounded-lg transition duration-200 text-lg"
            >
              {text}
            </button>
          ))}
